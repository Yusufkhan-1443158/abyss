...
BUNDLE_DIR_V1 = ROOT / "backend" / "models" / "dl_pro"
BUNDLE_DIR_V2 = ROOT / "backend" / "models" / "dl_pro_v2"
BUNDLE_DIR_V3 = ROOT / "backend" / "models" / "dl_pro_v3"
...
def build_features_v2(s2: dict, k_clusters: int = 5) -> np.ndarray:
    ...
    spm, ndti_arr, _, oh = turbidity_features(s2, k_clusters=k_clusters)
    spm = np.log1p(spm)
...
def build_features(s2: dict) -> np.ndarray:
    bundle = load_bundle()
    if bundle["meta"].get("in_dim", 16) >= 22:
        return build_features_v2(s2)
...
def _predict(model: SDBMLPHead, X: np.ndarray, mu: np.ndarray, sd: np.ndarray,
    ...
    # Keep dropout ON, linear OFF.
    for m in model.modules():
        if isinstance(m, nn.Dropout):
            m.train()
        elif isinstance(m, nn.Linear):
            m.eval()
    samples = np.zeros((n_mc, len(X)), dtype=np.float32)
    with torch.no_grad():
        for s in range(n_mc):
            samples[s] = model(Xt).numpy()
...
def predict_dl_pro(s2: dict,
    ...
    # Inference w/ MC-Dropout uncertainty ─────────────────────────────
    flat = feats.reshape(-1, F)
    flat_n = (flat - feat_mean) / np.where(feat_std < 1e-3, 1e-3, feat_std)
    pred_mean, pred_std = _predict(model, flat_n.astype(np.float32),
                                     feat_mean, feat_std, n_mc=n_mc)
    depth_pred = pred_mean.reshape(H, W).astype(np.float32)
    sigma      = pred_std.reshape(H, W).astype(np.float32)
    ...
    try:
        try:
            from backend.turbidity import (turbidity_features, sigma_total,
                                              prob_within_iho)
        except ImportError:
            from turbidity import (turbidity_features, sigma_total,    # type: ignore
                                     prob_within_iho)
    ...
    wm = s2.get("water_mask", np.ones((H, W), dtype=bool))
